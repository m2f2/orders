        ...
        <div className="flex flex-nowrap gap-1 ">
          {activeOrders.map((order) => (
            ...
            <div
              ...
              className={`
                flex items-center justify-center px-4 py-1 transition-all duration-200 cursor-pointer min-w-[140px] bg-white  border-l
                ${selectedOrder === order.id ? "text-[#7A8699] " : "text-[#C7CED9]"}
              `}
            >
              <div className="flex flex-col">
                <span className="text-sm text-center truncate max-w-[216px] ">
                  {order.title}
                </span>
                <span className="text-xs text-[#C7CED9]">رقم الطلب: {order.id}</span>
              </div>
              <X
